...
output_file = "/data/kebl6672/llm_jailbreak/translated_results/decrypt_cipher_gemma2_empty_assistant.csv"
...
for chunk_idx, df_chunk in enumerate(df_chunks):
    ...
    for column_prefix in ["harmful_prompt", "harmless_prompt"]:
        for cipher in cipher_list:
            input_col = f"{column_prefix}_{cipher}"  
            ground_truth_col = f"{column_prefix}"  
            ...
            # Matches keys for decryption
            prompt_type = f"{cipher}_to_plain"

            # Translate text in batch
            input_texts = df_chunk[input_col].astype(str).tolist()
