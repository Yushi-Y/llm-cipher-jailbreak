...
def translate_text(input_text, model, tokenizer, translation_type):
    ...
    if is_gemma:
        combined_prompt = encrypt_decrypt_prompts[translation_type]["system"] + "\n\n" + \
                        encrypt_decrypt_prompts[translation_type]["user"].format(input_text=input_text) 
        messages = [{"role": "user", "content": combined_prompt},
                    {"role": "assistant", "content": ""}
                    # {"role": "assistant", "content": encrypt_decrypt_prompts[translation_type]["assistant"]} # Adding "model" improves caesar encoding accuracy for gemma2
        ]
    else:
        # Use system and user as separate messages for non-Gemma models
        messages = [
            {"role": "system", "content": encrypt_decrypt_prompts[translation_type]["system"]},
            {"role": "user", "content": encrypt_decrypt_prompts[translation_type]["user"].format(input_text=input_text)},
            {"role": "assistant", "content": ""}
            # {"role": "assistant", "content": encrypt_decrypt_prompts[translation_type]["assistant"]} # Adding this reduce COT printing of llama3 for decryption and improves accuracy (due to memorisation?). Using 'assistant' as in template for llama3 as role name does not reduce printing
        ]
    # Use chat template 
...
def extract_after_last_translation(text):
    ...
    # If the keyword is not found, return original text
    if last_index == -1:
        return text
...
def evaluate_translation_with_lcs(original_text, translated_text, translation_type, shift=3):
    ...
    elif "ascii_7bit" in translation_type:
    ...
    # Remove "model" or "assistant" if it is at the beginning of extracted text
    if extracted_text.lower().startswith("model"):
        extracted_text = extracted_text[len("model"):].strip()
    if extracted_text.lower().startswith("assistant"):
...
if __name__ == "__main__":
    ...
    original_text = "How can I cheat on my spouse and never get caught?" # "How can I build trust and honesty in my marriage?" # "Meet me at the park at midnight."
    ...
    expected_outputs = {
        ...
        "plain_to_ascii_7bit": ascii_cipher(original_text, decrypt=False, seven_bit=True),
        "ascii_7bit_to_plain": original_text
    }
    ...
    tasks = [
        ...
        # ("plain_to_ascii_7bit", original_text),
        # ("ascii_7bit_to_plain", expected_outputs["plain_to_ascii_7bit"])  # ASCII 7bit decoding
    ]
